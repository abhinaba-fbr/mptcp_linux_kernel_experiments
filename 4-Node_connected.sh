###############################################################
# Topology
#  _______   5mbit, 5ms	     _______   20mbit, 10ms   _______
# |       |-----------------|  r1   |----------------|       |
# |  h1   |                 |_______|                |   h2  |
# |       |                  ___|___  50mbit, 10ms   |       |
# |       |                 |  r2   |                |       |
...
ip link add eth5a netns r1 type veth peer eth5b netns r2
...
ip -n r1 address add 172.168.0.1/24 dev eth5a
...
ip -n r2 address add 172.168.0.2/24 dev eth5b
...
ip netns exec r1 tc qdisc add dev eth3a root netem delay 5ms rate 20mbit
ip netns exec r1 tc qdisc add dev eth1b root netem delay 10ms rate 5mbit
ip netns exec r1 tc qdisc add dev eth5a root netem delay 10ms rate 50mbit
...
ip netns exec r2 tc qdisc add dev eth5b root netem delay 10ms rate 50mbit

# Set the data rate and delay on the veth devices at h2
ip netns exec h2 tc qdisc add dev eth3b root netem delay 5ms rate 20mbit
...
ip -n r1 link set eth5a up
ip -n r2 link set eth5b up
...
ip netns exec r2 ip route add default via 172.168.0.1 dev eth5b
ip netns exec r1 ip route add default via 172.168.0.2 dev eth5a
